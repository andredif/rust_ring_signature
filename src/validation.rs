...
pub fn voter_to_json(voter: Voter) -> Result<JSONVoter, serde_json_wasm::de::Error> {
...
pub fn vote_to_json(vote: Vote) -> Result<JSONVote, serde_json_wasm::de::Error> {
...
pub fn vote_to_string(vote: Vote) -> Result<String, serde_json_wasm::ser::Error> {
    let json_val = vote_to_json(vote).unwrap();
    serde_json_wasm::to_string(&json_val)
}

pub fn voter_to_string(voter: Voter) -> Result<String, serde_json_wasm::ser::Error> {
    let json_val = voter_to_json(voter).unwrap();
    serde_json_wasm::to_string(&json_val)
}


pub fn all_voters_to_string(voters: Vec<Voter>) -> Result<String, serde_json_wasm::ser::Error> {
    let mut json_voters: Vec<JSONVoter> = Vec::new();
    for voter in voters {
        let json_voter = voter_to_json(voter).unwrap();
        json_voters.push(json_voter);
    }
    serde_json_wasm::to_string(&json_voters)
}

pub fn str_to_all_voters(json_str: &str) -> Result<Vec<Voter>, serde_json_wasm::de::Error> {
    let json_values: Vec<JSONVoter> = serde_json_wasm::from_str(json_str)?;
...
pub fn str_to_vote(json_str: &str) -> Result<Vote, serde_json_wasm::de::Error> {
    let json_val: JSONVote = serde_json_wasm::from_str(json_str)?;
...
pub fn str_to_voter(json_str: &str) -> Result<Voter, serde_json_wasm::de::Error> {
    let json_val: JSONVoter = serde_json_wasm::from_str(json_str)?;
    let pub_keys = string_to_pub_keys(json_val.pub_keys).unwrap();
    let identifier = json_val.identifier;
...
pub fn string_to_pub_keys(pubkeys: String) -> Result<Vec<CompressedRistretto>, serde_json_wasm::de::Error> {
    Ok(deserialize(&base64::decode(&pubkeys).unwrap_or_default()).unwrap_or_default())
}

pub fn pub_keys_to_string(pub_keys: Vec<CompressedRistretto>) -> Result<String, serde_json_wasm::de::Error> {
